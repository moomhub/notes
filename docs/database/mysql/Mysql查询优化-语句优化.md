...
> 使用MySQL 官网提供的测试数据库来进行演示（DVD租赁公司）
...
> 覆盖索引通过取消回表操作，提升查询效率，若数据的查询不只使用了一个索引，则不是覆盖索引，可以通过优化SQL语句或优化联合索引，来使用覆盖索引
...
MySQL在选取索引时，会参考索引的基数(Cardinality)、基数是MySQL估算的，反映这个字段有多少种取值

> 如果数据量过大，MySQL会自动选取几个页算出取值的平均值，再乘以页数，即为基数
...
count(索引字段)，可以覆盖，依然需要每次判断字段是否为null，count(主键)，同理

### count(1)

count(1)只有扫描索引树，没有解析数据行的过程，理论更快，但Server层依然每次需要判断“1是否为null"

### count(*)

`count(*)`一般用来返回数据表行数
MylSAM的`count(*)`直接返回数据库中记录的数据表行数，由于InnoDB支持事务，数据库中不记录数据表行数
...
优化内存占用 :减少sort_buffer_size  减少内存 直接走硬盘数据(不推荐)
...
SQL执行过程中出现了两次中间结果，都是全长度的、仅需要一个随机结果，缺经历了不必要的排序（虽然优化过)，调用了很多次RAND()。
...
MySQL5.6之前，需要先使用索引查询store_id in (1,2)再全部回表验证film_id = 3，MySQL5.6之后，如果索引中可以判断，直接使用索引过滤。
...
MySQL中若出现字符串与数字比较，会将字符串转换为数字。**字符  数字转字符、数字  字符转数字**，也会出现无法通过索引来查找.
...
MySQL中,utf8与utf8mb4字段比较时，会把utf8转为utf8mb4上一页的SQL相当于:
...
排序偏移量大时，会丢弃大量无用数据，导致效率低下，可以采取先索引覆盖，再用最终ID回表的方法，优化效率
